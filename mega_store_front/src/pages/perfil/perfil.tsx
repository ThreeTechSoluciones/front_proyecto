...
const Perfil: React.FC = () => {
  ...
  const manejarSeleccionDeArchivo = async (
    e: React.ChangeEvent<HTMLInputElement>
  ) => {
    const archivo = e.target.files?.[0];
    if (archivo) {
      if (archivo.type.startsWith("image/")) {
        //se analiza si el archivo supera 5mb
        try {
          // Comprimir la imagen
          const opciones = {
  ...
  return (
    <div className={Style.screen}>
      <div className={Style.container}>
        <h1>MI PERFIL</h1>
        <ImageAvatars
          src={imagenPerfil || "/broken-image.jpg"}
          alt="Foto perfil"
          width="130px"
          height="130px"
        />
        <a className={Style.image} onClick={handleMostrarBoton}>
          Editar imagen
        </a>
        <div className={mostrarBoton ? Style.show : Style.hide}>
          <input
            ...
            accept="image/*"
          />
          <button onClick={handleSubirArchivo} className={Style.button}>
            Seleccionar Archivo
          </button>
        </div>
          ...
          >
            ...
            <div className={Style.input}>
              <TextField
                ...
                defaultValue={
                  datosPerfil?.direccionEnvio || "Dirección no disponible"
                }
                variant="standard"
                slotProps={{
                  input: {
              ...
              {errors.direccionEnvio &&
                typeof errors.direccionEnvio.message === "string" && (
                  <p className={Style.alerts}>
            ...
            <div className={Style.input}>
              ...
              {errors.numeroTelefono &&
                typeof errors.numeroTelefono.message === "string" && (
                  <p className={Style.alerts}>
