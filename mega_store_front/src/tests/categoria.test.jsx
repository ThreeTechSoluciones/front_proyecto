...
import { newCategoria, getCategorias } from "../service/CategoriaService";
...
describe("Categoría component", () => {
  ...
  test("U2F-001: Validar caracteres no permitidos", async () => {
    ...
    expect(alert).toHaveTextContent(
      "Categoría contiene caracteres no permitidos."
    );
  });
  ...
  test("U2F-003: Registrar categoría con éxito", async () => {
    ...
    expect(
      await screen.findByText("Categoría Remeras registrada")
    ).toBeInTheDocument();
  });
  ...
  test("U2F-006: Validar longitud de campo.", async () => {
    ...
    const alert = await screen.findByTestId("CategoryAlerts");
    expect(alert).toHaveTextContent(
      "Categoría no puede superar los 100 caracteres"
    );
  });
